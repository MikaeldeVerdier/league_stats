...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            assert mode in (1, 2), "Invalid mode entered."

            if mode == 1:
                participant_ids = [val["participantId"] for match in match_timelines for val in match["info"]["participants"] if val["puuid"] == summoner_info["puuid"]]

                formatted_infos = "\n".join([f"{i}: {key}" for i, key in enumerate(infos[0].keys())])
                infos_indices = input(f"Choose infos:\n{formatted_infos}\n")
                parsed_selected_infos = [int("".join([x for x in substr if x.isdigit()])) for substr in infos_indices.split(",")]
                assert max(parsed_selected_infos) < len(infos[0]), f"Invalid info entered."

                res = [np.mean([list(info.values())[selected_index][participant_id - 1] for info, participant_id in zip(infos, participant_ids)]) for selected_index in parsed_selected_infos]
                print(res)
            else:
